...
let handler = async (m, {
    ...
    if (/image/g.test(mime) && !/webp/g.test(mime)) {
        ...
        const img = await q.download()
                ...
                await conn.sendFile(m.chat, Buffer.from(buffer), '', '*Made by:* ', m)
            ...
            try {
                ...
                await conn.sendFile(m.chat, url, 'removebg.png', '*Made by: i', m)
            } catch (e) {
                let img = await webp2png(await q.download()),
                url = API('lolhuman', '/api/removebg2', {
                    img
                }, 'apikey')
            await conn.sendFile(m.chat, url, 'removebg.png', 'i', m)
            }
        }
    } else {
